...
def tiny_imagenet_train_preprocess(input_size, normalize=__imagenet_stats):
    normalize = transforms.Normalize((0.4802, 0.4481, 0.3975), (0.2770, 0.2691, 0.2821))
    return transforms.Compose(
        [transforms.RandomResizedCrop(input_size), transforms.RandomHorizontalFlip(), transforms.ToTensor(),
         normalize, ])

def tiny_imagenet_test_preprocess(input_size, normalize=__imagenet_stats):
    normalize = transforms.Normalize((0.4802, 0.4481, 0.3975), (0.2770, 0.2691, 0.2821))
    return transforms.Compose([transforms.Resize(input_size), transforms.ToTensor(), normalize, ])
...
def get_transform(name='imagenet', input_size=None,
    ...
    if name == 'imagenet':
        ...
        if augment:
            return tiny_imagenet_train_preprocess(input_size, normalize=normalize)
        else:
            return tiny_imagenet_test_preprocess(input_size, normalize=normalize)
    elif 'cifar' in name:
